...
namespace B83 {
	public class TestBehaviour : MonoBehaviour {
		public static Run InputPopUp(string aTitle, string aStartValue, string aOkString, string aCancelString,
			System.Action<string> aOnUpdateValue) {
		...
		public static Run IntInputPopUp(string aTitle, int aStartValue, string aOkString, string aCancelString,
			System.Action<int> aOnUpdateValue) {
		...
		public static Run FloatInputPopUp(string aTitle, float aStartValue, string aOkString, string aCancelString,
			System.Action<float> aOnUpdateValue) {
			return InputPopUp(aTitle, aStartValue.ToString(), aOkString, aCancelString, (str) => {
				float val = 0;
		...
		private static IEnumerator Wizard() {
			...
			float duration = 0f;
			yield return FloatInputPopUp("input message duration", 5, "ok", "use default", (val) => duration = val)
				.WaitFor;
			...
			yield return Run.After(delay, () => {
				...
				window = Run.CreateGUIWindow(r, "Scheduled Message", (win) => { GUILayout.Label(message); });
				...
				if (GUILayout.Button("open window")) {
					...
					Run.CreateGUIWindow(new Rect(10, 10, 200, 200), "Sample Window", (win) => {
						...
						if (GUILayout.Button("open popup")) {
							...
							float distance = 10;
							var popUpWin = Run.CreateGUIWindow(new Rect(100, 100, 200, 300), "Popup Window",
								(popup) => {
									if (closeAction != null) {
									...
									if (linkGameObject == null) {
										if (GUILayout.Button("link"))
											linkGameObject = Run.EachFrame(() => {
												Vector3 P = new Vector3(popup.pos.x, Screen.height - popup.pos.y,
													distance);
									...
									if (GUILayout.Button("Lerp")) {
										Run.Lerp(1,
											(t) => { windowObject.localEulerAngles = new Vector3(0, t * 90, 0); });
									}
